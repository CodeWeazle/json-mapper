...
/**
 * class generator interface for JsonMapper with some helpful default
 * implementations
 */
public interface ClassGenerator {
	 ...
	 *                       created
	 */
	default void createNoArgsConstructor(ElementInfo annotationInfo, Map<String, MethodSpec> methods) {
	 ...
	 *                       created
	 */
	default void createSetterMethodSpec(VariableElement field, ElementInfo annotationInfo, TypeName fieldTypeName,
		...
		if (type.getKind() == TypeKind.DECLARED) {
			...
			if (typeArguments != null && typeArguments.size() > 0) {
				...
				// List
				if (type != null && getTypeUtils().isAssignable(getTypeUtils().erasure(type),
						getTypeUtils().erasure(collectionType))) {
					setterBuilder.addStatement("this.$L = new $T" + typeArgs + "()", field.getSimpleName().toString(),
							ArrayList.class).addStatement("this.$L.addAll($L)", field.getSimpleName().toString(),
									field.getSimpleName().toString());
					// Set
				} else if (type != null
						&& getTypeUtils().isAssignable(getTypeUtils().erasure(type), getTypeUtils().erasure(setType))) {
					setterBuilder.addStatement("this.$L = new $T" + typeArgs + "()", field.getSimpleName().toString(),
							HashSet.class).addStatement("this.$L.addAll($L)", field.getSimpleName().toString(),
									field.getSimpleName().toString());
					// Map
				} else if (type != null
						&& getTypeUtils().isAssignable(getTypeUtils().erasure(type), getTypeUtils().erasure(mapType))) {
					setterBuilder.addStatement("this.$L = new $T" + typeArgs + "()", field.getSimpleName().toString(),
							HashMap.class).addStatement("this.$L.putAll($L)", field.getSimpleName().toString(),
									field.getSimpleName().toString());
				}
			} else {
			...
			setterBuilder.addStatement("this.$L = $L.clone()", field.getSimpleName().toString(),
					field.getSimpleName().toString());
		} else if (type.getKind() != TypeKind.PACKAGE && type.getKind() != TypeKind.MODULE
				&& type.getKind() != TypeKind.ERROR && type.getKind() != TypeKind.EXECUTABLE
				&& type.getKind() != TypeKind.UNION && type.getKind() != TypeKind.NULL) {
			setterBuilder.addStatement("this.$L = $L", field.getSimpleName().toString(),
					field.getSimpleName().toString());
		}
	 ...
	 *                       created
	 */
	default void createGetterMethodSpec(final VariableElement field, final ElementInfo annotationInfo,
			final TypeName fieldTypeName, final Map<String, MethodSpec> methods) {
		// create getter method
		String getterName = generateGetterName(annotationInfo, field.getSimpleName().toString(),
				fieldTypeName.toString().equals(Boolean.class.getName()));
		...
		MethodSpec getter = MethodSpec.methodBuilder(getterName).addModifiers(Modifier.PUBLIC)
				.addStatement("return " + fieldName).returns(fieldType).build();
	 ...
	 */
	default FieldSpec createFieldSpec(VariableElement field, ElementInfo annotationInfo, TypeName fieldClass,
	...
	default String generateGetterName(ElementInfo annotationInfo, String fieldName, Boolean isBoolean) {
		if (annotationInfo.fluentAccessors()) {
			...
			return ((isBoolean != null && isBoolean.equals(Boolean.TRUE)) ? "is" : "get")
					+ StringUtil.capitalise(fieldName);
		}
	}

	/**
	 * generate a name for the field's setter method according to the specified
	 * method. (fluent or not)
	 * 
	 * @param annotationInfo - information about the annotation arguments
	 * @param fieldName      - name of the field to create the setter's name for
	...
	/**
	 * check if field is declared as final static private
	 * 
	 ...
	 * @param type            - the {@code GenerationType} enum value which
	 *                        specifies the type that needs to be checked on the
	 *                        {@code Mapped} annotation
	 * @return true if present
	 */
	default boolean fieldIsAnnotedWith(final Element field, Class<?> annotationClazz, GeneratorType type) {
	 ...
	 * @param type            - {code GeneratorType} defining the type of generator
	 *                        used (POJO,JSON,XML)
	 * @return boolean which indicates whether or not the given typeElement is
	 *         annotated with the given class.
	 */
	@SuppressWarnings("unchecked")
	default boolean typeIsAnnotatedWith(final TypeElement typeElement, final Class<?> annotationClazz,
			GeneratorType type) {

		boolean isAnnotated = false;
		if (annotationClazz.isAnnotation()) {
			isAnnotated = (typeElement != null
					&& Arrays.asList(typeElement.getAnnotationsByType((Class<? extends Annotation>) annotationClazz))
							.stream().anyMatch(annotation -> ((Mapped) annotation).type().equals(type)));
		}
		return isAnnotated;
	...
	/**
	 * checks the given if {@code TypeName} belongs to some kind of
	 * {@code Collection}, {@code Set} or {@code Map} and returns the
	 * {@code ParameterizedTypeName} of the field.
	 * 
	 * @param annotationInfo - information about the annotation arguments
		...
		if (type.getKind() == TypeKind.DECLARED) {
			...
			if (type != null && getTypeUtils().isAssignable(getTypeUtils().erasure(type),
				...
				// Map
					...
					&& getTypeUtils().isAssignable(getTypeUtils().erasure(type), getTypeUtils().erasure(mapType))) {
				fieldType = ParameterizedTypeName.get(ClassName.get(Map.class), destinationTypeArguments.get(0),
						destinationTypeArguments.get(1));
			}
		}
		return fieldType;
	}
	 ...
	 * @return a {@code List} of {@code TypeName} instances containing the type
	 *         arguments of the given type. Empty if none are present.
	 */
	default List<TypeName> obtainTypeArguments(TypeMirror type) {
		...
		if (typeArgumentMirrors.size() > 0) {
			typeArguments
					.addAll(typeArgumentMirrors.stream().map(arg -> TypeName.get(arg)).collect(Collectors.toList()));
		}
		return typeArguments;
	}

	/**
	 * Collect types of a parametrized field and return mapped type if any of them
	 * is annotated with with the mapping annotation itself.
	 * 
	 * @param annotationInfo - information about the annotation arguments
	 * @param typeArguments  - {@code List} of {@code TypeName} instances containing
	 *                       the (source) type arguments
	 * @return List of collected {@code TypeName} instances containing the (possibly
	 *         mapped) types.
	 */
	public List<TypeName> collectTypes(ElementInfo annotationInfo, List<TypeName> typeArguments);

	/**
	 * generates the package name base on the given annotation arguments
	 * 
	 * @param key            {@code ClassName} instance of the class to generate the
	 *                       package name from
	 * @param annotationInfo - {@code ElementInfo} instance to retrieve the given
	 *                       sub-package name from.
	 * @return package name considering the given package and sub-package name in
	...
	/**
	 * allow access to the {@code Types} provided to the {@code ClassGenerator}
	 * sub-classes constructor
	 * 
	 * @return {@code Types} provided to the {@code ClassGenerator} sub-classes
	 *         constructor
	 */
	public Types getTypeUtils();

	/**
	 * allow access to the {@code Elements} provided to the {@code ClassGenerator}
	 * sub-classes constructor
	 * 
	 * @return {@code Elements} provided to the {@code ClassGenerator} sub-classes
	 *         constructor
	 */
	 ...
	 * 
	 * @param field which is to be checked whether it is of an annotated class or
	 *              not.
